...
from models import ChatMessageRequest, ChatMessageOut, OkResponse
from dependencies import get_current_user, decode_token
from state import VALID_CITIES, get_state
...
async def _ai_mood_reply(city: str, user_id: int, text: str):
    ...
    """
    try:
        from services import psychotype as _pt, assistant as _as
...
async def _handle_studio(websocket: WebSocket, user: dict, city: str, data: dict):
    ...
    try:
        from services import psychotype as _pt
        analysis = await _pt.analyze(text)
        await db.execute(
            """
            INSERT INTO psychotypes
